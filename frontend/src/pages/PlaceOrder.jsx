  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      for (const productId in cartItems) {
        const sizes = cartItems[productId]; // Get sizes object for a product
        ...
        for (const size in sizes) {
          ...
          if (quantity > 0) {
            ...
            if (itemInfo) {
              itemInfo.size = size; // Add the size to the item info
              itemInfo.quantity = quantity; // Add the quantity to the item info
              orderItems.push(itemInfo); // Add to the order items array
            }
          }
        }
      }
    ...
    <form onSubmit={onSubmitHandler} className="flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t">
      <div className="flex flex-col gap-4 w-full sm:max-w-[480px]">
        ...
        <div className="flex gap-3">
          ...
          <input required onChange={onChangeHandler} name="state" value={formData.state} className="border border-gray-300 rounded py-1.5 px-3.5 w-full" type="text" placeholder="State/Division" />
        </div>
        <div className="flex gap-3">
      ...
      <div className="mt-8">
        ...
        <div className="mt-12">
          ...
          <div className="flex gap-3 flex-col lg:flex-row">
            ...
            <div onClick={() => setMethod('sslcommerz')} className="flex items-center gap-3 border p-2 px-3 cursor-pointer">
              <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'sslcommerz' ? 'bg-green-400' : ''}`}></p>
              <img className="h-5 mx-4" src={assets.sslcommerz_logo} alt="sslcommerz logo" />
            </div>
